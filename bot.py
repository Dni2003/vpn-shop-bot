...
async def support_command(message: Message):
    support_user = await get_setting("support_username") or "Dni2003"
    support_hours = await get_setting("support_hours") or "۹ صبح تا ۱۲ شب"
    
    await message.answer(
        f"📞 پشتیبانی:\n\n"
        f"برای ارتباط با ادمین، از لینک زیر استفاده کن:\n"
        f"@{support_user}\n\n"
        f"⏰ پاسخگویی: {support_hours}"
    )
