...
async def process_purchase(message: Message, state: FSMContext, callback: CallbackQuery = None):
    ...
    try:
        ...
        if balance < final_price:
            ...
            if callback:
                await callback.message.edit_text(msg, reply_markup=back_to_main_keyboard())
            else:
                await message.answer(msg, reply_markup=back_to_main_keyboard())
            await state.clear()
            return
        ...
        if callback:
            await callback.message.edit_text(error_msg, reply_markup=back_to_main_keyboard())
        else:
            await message.answer(error_msg, reply_markup=back_to_main_keyboard())
        await state.clear()
        ...
        try:
            if message.text:
                ...
                sent_msg = await bot.send_message(
                    admin_id,
                    f"📩 پیام جدید از {user_name} (@{username}) [ID: {user_id}]:\n\n"
                    f"(نوع پیام قابل نمایش نیست، لطفاً در تلگراف مشاهده کنید)"
                )
                await save_admin_reply(sent_msg.message_id, user_id)
